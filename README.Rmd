...
```{r plot super-cell purity in terms of cell-line annotation}
purity.df <- plot_purity(
  SC.list, 
  annotation.name = 'cell_line'
  )
```
...
```{r}
table(purity.df$Gamma_actual)
```
```{r}
library(ggplot2)

ggplot(purity.df, aes(x = factor(Gamma_actual), y = Purity, color = Method, alpha = factor(Seed))) + 
  geom_boxplot() 
```
